    ...
    let resolve_cef_dir = |location: &Path| -> anyhow::Result<PathBuf> {
        ...
        if !fs::exists(&cef_dir)? {
            ...
            let index = {
                let mut attempt = 0;
                loop {
                    match CefIndex::download_from(&download_url) {
                        Ok(index) => break index,
                        Err(error) if attempt < 5 => {
                            attempt += 1;
                            eprintln!(
                                "CEF index download failed ({error}); retrying ({attempt}/5)"
                            );
            ...
            let archive = {
                let mut attempt = 0;
                loop {
                    match version.download_archive_from(&download_url, location, false) {
                        Ok(archive) => break archive,
                        Err(error) if attempt < 5 => {
                            attempt += 1;
                            eprintln!(
                                "CEF archive download failed ({error}); retrying ({attempt}/5)"
                            );
                            thread::sleep(Duration::from_secs(attempt * 2));
                        }
                        Err(error) => return Err(error.into()),
