...
impl<T: UserEvent> CefRuntime<T> {
    fn init(
        ...
        let resources_dir_path = cef_config
            .resources_dir_path
            .as_ref()
            .map(|path| path.to_string_lossy().into_owned());
        let locales_dir_path = cef_config
            .locales_dir_path
            .as_ref()
            .map(|path| path.to_string_lossy().into_owned());
        let settings = cef::Settings {
            ...
            resources_dir_path: resources_dir_path.as_deref().unwrap_or_default().into(),
            locales_dir_path: locales_dir_path.as_deref().unwrap_or_default().into(),
